...
if nargin>4, edc = extradevcode; else, edc = []; end;

global remotecomputertypeNLT;

if strcmp(computer,'LNX86')&strcmp(remotecomputertypeNLT,'MAC2'),
	pathname = linpath2mac(pathname);
end;
...
if saveWaves, % save the stimulus timings;
  ...
  fprintf(fid,'cd(gggg);\n');
  fprintf(fid,'pause(max([0 10-etime(clock,zzz)]));snd(''play'',''glass'');snd(''play'',''glass'');snd(''play'',''glass'');\n');
...
fprintf(fid,'else, snd(''play'',''glass'');snd(''play'',''glass'');');
