   ...
   case 'DetailCB',
	...
	if b,
	  ...
	  if v==1,
	    ...
	    cent=vals{9};
	    x=vals{6}-18.0975+cent(1)/pixels_per_cm;
	    y=vals{7}; 
	    z=vals{8}+32.0675-cent(2)/pixels_per_cm;
	    rf = [atan(x/y) atan(z/y)]*180/pi;
	    set(ft(thefig,'RFpos'),'String',mat2str(rf,3),...
			      'userdata',rf);
	    diameter = vals{10};
	    lefttoright=(atan( (x+diameter(2)/2/pixels_per_cm)/y) ...
		- atan( (x-diameter(2)/2/pixels_per_cm)/y))*180/pi;
	    toptobottom=(atan( (z+diameter(1)/2/pixels_per_cm)/y) ...
		- atan( (z-diameter(1)/2/pixels_per_cm)/y))*180/pi;
