...
initscreensize=[800 600];

if nargin==0,
	z = geteditor('screentool');
	if ~isempty(z), figure(z); return; end; % we've already got one, bring it to the front
	fig=figure;
	set(fig,'MenuBar','none','name','Screen Tool','NumberTitle','off');
...
elseif ~ischar(figNum),  % is the call described above
        fig = figNum; figure(fig); clf; set(fig,'MenuBar','none','name','Screen Tool');
	...
	sr=[0 0 screentoolstruct.screensize(1) screentoolstruct.screensize(2)];
		...
		case 'GetRectRemote',
			strs={'RFcurrent = [0 1 0 1]; CloseStimScreen;CloseMonitorScreen; CloseStimScreen; CloseMonitorScreen; try, RFcurrentrect = RFmap; end; ShowStimScreen;'
			   'save(''fromremote'',''RFcurrentrect'',''-mat'');'
			   'save(''gotit'',''RFcurrentrect'',''-mat'');'};
			strs={'global quickRFrect;'
			   'save(''fromremote'',''quickRFrect'',''-mat'');'
			   'save(''gotit'',''quickRFrect'',''-mat'');'};
			p = getrempath;
			if isempty(p),
				errordlg('Could not find RunExperiment window');
			else,
		...
		case 'SetRectRemote',
			...
			if ~isempty(r),
				...
				else, b=sendremotecommand(str); end; %no input
		...
		case 'RFstart',
			...
			pr = getrempath;
			if ~isempty(pr), b=sendremotecommand(str); end;
		case 'RFstop',
			str = {'ReceptiveFieldGlobals; cd(pwd);'
			       'save(''gotit'',''RFcurrentrect'',''-mat'');'
				'cd(pwd);'
...
currrect = uicontrol('Parent',h0, ...
        ...
        'Tag','EditText1',...
	'HorizontalAlignment','left',...
	'Callback','screentool plotcurr','String','[0 0 800 600]');
