function [varargout]=hartleysheetlet_process(fig, typeName, ds, command, varargin)
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [rev1s,t1s,rev2s,t2s,ctredit,onoff,cb1,cb2,cb3,refCtrl]=hartleysheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
    ...
    case 'CoarseRunBt',
        ...
        if cb3,
        	thescript = longhartley(ctredit(1),ctredit(2));
        end;
    ...
    case 'FineRunBt',
    	...
    	if cb3,
        	thescript = longhartley(rev1s,ctredit(2),ctredit(3));
    	end;
        if nargout>0, varargout{1}=thescript; end;        
        test=RunScriptRemote(ds,thescript,saveit,0,1);
    ...
    case 'CoarseEditBt',
        ...
        if ~isempty(rev1s),
            hartleysheetlet_process(fig, typeName, ds, [typeName 'SetVars'],rev1s,[],[],[],[],[],[],[],[]);
        end;
    case 'FineEditBt',
        cl = class(rev2s);
        if ~isempty(cl)&~strcmp(cl,'double'), 
        ...
        if ~isempty(rev2s),
            hartleysheetlet_process(fig, typeName, ds, [typeName 'SetVars'],[],[],rev2s,[],[],[],[],[],[]);
        end;
    case 'CoarseCB',
    case 'FineCB',
    case 'CenterLocCB',
    case 'AnalyzeCoarseBt',
        ...
        if ~isempty(mycell),
	    errordlg('No analysis for Hartley stims just yet.');
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['hartleysheet' typeName],'-mat'); g=getfield(g,['hartleysheet' typeName]);
        hartleysheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['hartleysheet' typeName '={rev1s,t1s,rev2s,t2s,ctredit,onoff,cb1,cb2,cb3};']);
        eval(['save ' fname ' hartleysheet' typeName ' -append -mat']);
    case 'AddDBBt',
        if cb3,
