function [varargout]=blinkingstimscriptsheetlet_process(fig, typeName, ds, command, varargin)
 % var input argument order list: revstim1, test1, revstim2, test2, durations
 % pausedurations, repinfo, cb1, cb2, refCtl
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [rev1s,t1s,rev2s,t2s,durations,pausedurations,repinfo,cb1,cb2,refCtrl]=blinkingstimscriptsheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
    case 'SetVars',
        ...
        if length(varargin)>4&~isempty(varargin{5}), set(findobj(fig,'tag',[typeName 'DisplayDurationsEdit']),'string',mat2str(varargin{5})); end;
        if length(varargin)>5&~isempty(varargin{6}), set(findobj(fig,'tag',[typeName 'PauseDurationsEdit']),'string',mat2str(varargin{6})); end;
        if length(varargin)>6&~isempty(varargin{7}), set(findobj(fig,'tag',[typeName 'RepInfoEdit']),'string',mat2str(varargin{7})); end;
        if length(varargin)>7&~isempty(varargin{8}), set(findobj(fig,'tag',[typeName 'FineCB']),'value',varargin{8}); end;
        if length(varargin)>8&~isempty(varargin{9}), set(findobj(fig,'tag',[typeName 'AnalyzeCoarseBt']),'userdata',varargin{9}); end;        
    case 'GetVars',
        ...
        if nargout>4, varargout{5}=str2num(get(findobj(fig,'tag',[typeName 'DisplayDurationsEdit']),'string')); end;
        if nargout>5, varargout{6}=str2num(get(findobj(fig,'tag',[typeName 'PauseDurationsEdit']),'string')); end;
        if nargout>6, varargout{7}=str2num(get(findobj(fig,'tag',[typeName 'RepInfoEdit']),'string')); end;
        if nargout>7, varargout{8}=get(findobj(fig,'tag',[typeName 'CoarseCB']),'value'); end;
        if nargout>8, varargout{9}=get(findobj(fig,'tag',[typeName 'FineCB']),'value'); end;
    ...
    case 'CoarseRunBt',
        ...
        rev1s = recenterstim(rev1s,{'rect',newrect,'screenrect',screenrect,'params',1,'constrain',0});
        foreachstimdolocal({'rev1s'},'recenterstim',{'rect',newrect,'screenrect',screenrect,'params',1,'constrain',0});
        thescript = makeblinkingstimscript(rev1s,durations,pausedurations,repinfo(1),repinfo(2),repinfo(3));
    ...
    case 'FineRunBt',
        [newrect,dist,screenrect] = getscreentoolparams;
        foreachstimdolocal({'rev2s'},'recenterstim',{'rect',newrect,'screenrect',screenrect,'params',1,'constrain',0});
        thescript = makeblinkingstimscript(rev2s,durations,pausedurations,repinfo(1),repinfo(2),repinfo(3));
        if nargout>0, varargout{1}=thescript; end;        
        test=RunScriptRemote(ds,thescript,saveit,0,1);
    ...
    case 'CoarseEditBt',
        ...
        if ~isempty(rev1s),
            blinkingstimscriptsheetlet_process(fig, typeName, ds, [typeName 'SetVars'],rev1s,[],[],[],[],[],[],[],[]);
        end;
    case 'FineEditBt',
        cl = class(rev2s);
        if ~isempty(cl)&~strcmp(cl,'double'), 
        ...
        if ~isempty(rev2s),
            blinkingstimscriptsheetlet_process(fig, typeName, ds, [typeName 'SetVars'],[],[],rev2s,[],[],[],[],[],[]);
        end;
    case 'CoarseCB',
    case 'FineCB',
    case 'CenterLocCB',
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['blinkingstimscriptsheet' typeName],'-mat'); g=getfield(g,['blinkingstimscriptsheet' typeName]);
        blinkingstimscriptsheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['blinkingstimscriptsheet' typeName '={rev1s,t1s,rev2s,t2s,durations,pausedurations,repinfo,cb1,cb2,refCtrl};']);
        eval(['save ' fname ' blinkingstimscriptsheet' typeName ' -append -mat']);
    case 'AddDBBt',
        if cb1|cb2,
