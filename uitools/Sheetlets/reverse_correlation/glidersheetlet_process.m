function [varargout]=glidersheetlet_process(fig, typeName, ds, command, varargin)
 % var input argument order list: revstim1, test1, revstim2, test2, ctrloc, 
 % onoffvalue, cb1, cb2, ctrcb, gliderEdit
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [rev1s,t1s,rev2s,t2s,ctredit,onoff,cb1,cb2,cb3,refCtrl,gliderEdit]=glidersheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
    case 'SetVars',
        ...
        if length(varargin)>10&~isempty(varargin{11}), set(findobj(fig,'tag',[typeName 'gliderEdit']),'string',varargin{11}); end;        
    case 'GetVars',
        ...
        if nargout>10, varargout{11}=get(findobj(fig,'tag',[typeName 'gliderEdit']),'string'); end;
    case 'CoarseRunBt',
	...
	gliderTypes = eval(gliderEdit);
	thescript = stimscript(0);
	for gt=1:length(gliderTypes),
		for thesign=[-1 1],
			p = getparameters(rev1s);
			p.correlator = gliderTypes(gt);
			p.correlator_sign = thesign;
		        if cb3,
				newscript = longsgs(glidergridstim(p),ctredit(1),ctredit(2),ctredit(3));
		        else,
				newscript=append(stimscript(0),glidergridstim(p));
			end;
			thescript = thescript + newscript;
		end;
	end;
	do = getDisplayOrder(thescript);
	thescript = setDisplayMethod(thescript,2,do(randperm(length(do))));
    ...
    case 'CoarseEditBt',
        ...
        if ~isempty(rev1s),
            glidersheetlet_process(fig, typeName, ds, [typeName 'SetVars'],rev1s,[],[],[],[],[],[],[],[]);
        end;
    case 'FineEditBt',
        cl = class(rev2s);
        if ~isempty(cl)&~strcmp(cl,'double'), 
        ...
        if ~isempty(rev2s),
            glidersheetlet_process(fig, typeName, ds, [typeName 'SetVars'],[],[],rev2s,[],[],[],[],[],[]);
        end;
    case 'CoarseCB',
    case 'FineCB',
    case 'CenterLocCB',
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['glidersheet' typeName],'-mat'); g=getfield(g,['glidersheet' typeName]);
        glidersheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['glidersheet' typeName '={rev1s,t1s,rev2s,t2s,ctredit,onoff,cb1,cb2,cb3,refCtrl,gliderEdit};']);
        eval(['save ' fname ' glidersheet' typeName ' -append -mat']);
    case 'AddDBBt',
        if cb3,
            if nargout>0, varargout{1} = []; end;
        else,
            warning('Warning: glider sheetlet was not checked.');
            if nargout>0, varargout{1} = []; end;
        end;
end;
