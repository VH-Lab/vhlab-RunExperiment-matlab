function [varargout]=doublegratingsheetlet_process(fig, typeName, ds, command, varargin)
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [testName,tuneEdit,parameter,tune2Edit,parameter2,gratCtrl,GoodCB,refCtrl]=doublegratingsheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
    case 'SetVars',
        ...
        if length(varargin)>4&~isempty(varargin{5}), set(findobj(fig,'tag',[typeName 'Title2Text']),'userdata',varargin{5});end;
        if length(varargin)>5&~isempty(varargin{6}), set(findobj(fig,'tag',[typeName 'Tune2Edit']),'userdata',varargin{6}); end;
    ...
    case 'GetVars',
        ...
        if nargout>4, varargout{5}=get(findobj(fig,'tag',[typeName 'Title2Text']),'userdata'); end;
        if nargout>5, varargout{6}=get(findobj(fig,'tag',[typeName 'Tune2Edit']),'userdata'); end;
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['doublegratingsheet' typeName],'-mat');g=getfield(g,['doublegratingsheet' typeName]);
        doublegratingsheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        tune2Edit,
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['doublegratingsheet' typeName '={testName,tuneEdit,parameter,tune2Edit,parameter2,gratCtrl,GoodCB,refCtrl};']);
        eval(['save ' fname ' doublegratingsheet' typeName ' -append -mat']);
    case 'AddDBBt',
        if get(findobj(fig,'tag',[typeName 'GoodCB']),'value')~=1,
            assoc = [];
        else,
            assoc = struct('type',[typeName ' test'],'owner','doublegratingsheetlet','data',testName,'desc',[typeName ' test']);
        end;
    ...
    case 'AnalyzeBt',
        ...
        if ~isempty(mycell),
            for j=1:length(mycell),
                ...
                assoc = struct('type',[typeName ' test'],'owner','doublegratingsheetlet','data',testName,'desc',[typeName ' test']);
                assoc(end+1) = struct('type',[typeName ' resp'],'owner','doublegratingsheetlet','data',co,'desc',[typeName ' resp']);
                db.assoc{j} = assoc;
            end;
        end;
