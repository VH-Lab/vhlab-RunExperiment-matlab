function [hlist,lengthwidth] = adaptgratingsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''adaptgratingsheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''adaptgratingsheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;
...
h = uicontrol(txt,'position',[upperLeft(1)+40-25 upperLeft(2)-30 200 20],'string',['AdaptParam T0 Ttop Ti AdChR'],'Tag',[typeName 'adaptTxt'], 'horizontalalignment', 'left');
hlist = [hlist h];
...
h = uicontrol(edit,'position',[upperLeft(1)+210 upperLeft(2)-30 400 20],'string','','Tag',[typeName 'adaptEdit']);
hlist = [hlist h];
