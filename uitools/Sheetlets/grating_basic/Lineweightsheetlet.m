function [hlist,lengthwidth] = Lineweightsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''lineweightsheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''lineweightsheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;
...
h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 100 20],'string','Line Weight','Tag',[typeName 'lineweighttxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];
