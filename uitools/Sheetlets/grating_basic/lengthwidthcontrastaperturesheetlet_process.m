function [varargout]=lengthwidthcontrastaperturesheetlet_process(fig, typeName, ds, command, varargin)
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [testName,lengthsstr,widthsstr,contraststr,gratCtrl,GoodCB,refsh]=lengthwidthcontrastaperturesheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
	...
	p.contrast = mycontrast(1);
        ...
        lwscript = makelengthwidthaperturetuning(base,lengths,widths);
	N = numStims(lwscript);
        for i=1:N,
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['lengthwidthcontrastaperturesheet' typeName],'-mat'); g=getfield(g,['lengthwidthcontrastaperturesheet' typeName]);
        lengthwidthcontrastaperturesheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['lengthwidthcontrastaperturesheet' typeName '={testName,lengthsstr,widthsstr,contraststr,gratCtrl,GoodCB,refsh};']);
        eval(['save ' fname ' lengthwidthcontrastaperturesheet' typeName ' -append -mat']);
    case 'AddDBBt',
        refsh='ref',
        if get(findobj(fig,'tag',[typeName 'GoodCB']),'value')~=1,
            assoc = [];
        else,
            assoc = struct('type',[typeName ' test'],'owner','lengthwidthcontrastaperturesheetlet','data',testName,'desc',[typeName ' test']);
        end;
    ...
    case 'AnalyzeBt',
		...
		convert_periodiccurve_2d(mypc,'length','contrast');
                assoc = struct('type',[typeName ' test'],'owner','lengthwidthcontrastaperturesheetlet','data',testName,'desc',[typeName ' test']);
                assoc(end+1) = struct('type',[typeName ' resp'],'owner','lengthwidthcontrastaperturesheetlet','data',co,'desc',[typeName ' resp']);
                db.assoc{j} = assoc;
            end;
        end;
