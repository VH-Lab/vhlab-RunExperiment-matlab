function [hlist,lengthwidth] = aperaturesheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''aperaturesheetlet_process'',''' typeName ''');'];
...
h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 90 20],'string',title,'Tag',[typeName 'LWtxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];

h = uicontrol(txt,'position',[upperLeft(1)+110 upperLeft(2)-7 80 20],'string','Lengths(pix):','Tag',[typeName 'Lengthtxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+190 upperLeft(2)-7 80 20],'string','pix','Tag',[typeName 'LengthsEdit']);
hlist = [hlist h];

h = uicontrol(txt,'position',[upperLeft(1)+270 upperLeft(2)-7 80 20],'string','Widths:','Tag',[typeName 'Widthtxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+350 upperLeft(2)-7 100 20],'string','width','Tag',[typeName 'WidthsEdit']);
hlist = [hlist h];
