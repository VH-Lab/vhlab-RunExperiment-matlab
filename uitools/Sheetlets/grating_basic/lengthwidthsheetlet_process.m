function [varargout]=lengthwidthsheetlet_process(fig, typeName, ds, command, varargin)
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [testName,lengthsstr,widthsstr,gratCtrl,GoodCB,refsh]=lengthwidthsheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['lengthwidthsheet' typeName],'-mat'); g=getfield(g,['lengthwidthsheet' typeName]);
        lengthwidthsheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['lengthwidthsheet' typeName '={testName,lengthsstr,widthsstr,gratCtrl,GoodCB,refsh};']);
        eval(['save ' fname ' lengthwidthsheet' typeName ' -append -mat']);
    case 'AddDBBt',
        refsh='ref',
        if get(findobj(fig,'tag',[typeName 'GoodCB']),'value')~=1,
            assoc = [];
        else,
            assoc = struct('type',[typeName ' test'],'owner','lengthwidthsheetlet','data',testName,'desc',[typeName ' test']);
        end;
    ...
    case 'AnalyzeBt',
        ...
        if ~isempty(mycell),
            for j=1:length(mycell),
                ...
                assoc = struct('type',[typeName ' test'],'owner','lengthwidthsheetlet','data',testName,'desc',[typeName ' test']);
                assoc(end+1) = struct('type',[typeName ' resp'],'owner','lengthwidthsheetlet','data',co,'desc',[typeName ' resp']);
                db.assoc{j} = assoc;
            end;
        end;
