function [hlist,lengthwidth] = brieforisheetlet(typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''brieforisheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''brieforisheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;

hlist = [];

h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-10 100 20],'string','Brief Ori','Tag',[typeName 'BriefOritxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];
