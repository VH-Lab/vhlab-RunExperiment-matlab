function [hlist,lengthwidth] = modulatingdoublegratingsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''modulatingdoublegratingsheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''modulatingdoublegratingsheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;
...
h = uicontrol(cb,'position',[upperLeft(1)+690 upperLeft(2)-28 10 10],'string','','Tag',[typeName 'Tune2WithModulatingGratingCB'],'visible','off');
hlist = [hlist h];
