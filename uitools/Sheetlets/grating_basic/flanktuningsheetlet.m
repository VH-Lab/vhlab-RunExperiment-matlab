function [hlist,lengthwidth] = flanktuningsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''flanktuningsheetlet_process'',''' typeName ''');'];
...
h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 100 20],'string','Flank tuning:','Tag',[typeName 'LWtxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];

h = uicontrol(cb,'position',[upperLeft(1)+110 upperLeft(2)-7 80 20], 'string', 'Iso Ori','tag',[typeName 'IsoOriCB']);

h = uicontrol(txt,'position',[upperLeft(1)+110+80 upperLeft(2)-7 130 20],'string','Flank location angle:','Tag',[typeName 'FlankLocationAngleTxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+110+80+130 upperLeft(2)-7 120 20],'string','angles','Tag',[typeName 'FlankLocationAngleEdit']);
hlist = [hlist h];
...
h = uicontrol(txt,'position',[upperLeft(1)+110+200 upperLeft(2)-30 80 20],'string','Num Flanks:','Tag',[typeName 'NumFlanksTxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+110+80+200 upperLeft(2)-30 50 20],'string','N','Tag',[typeName 'NumFlanksEdit']);
hlist = [hlist h];

h = uicontrol(txt,'position',[upperLeft(1)+110+80+50+200 upperLeft(2)-30 30 20],'string','C:','Tag',[typeName 'ContrastTxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+110+80+50+30+200 upperLeft(2)-30 70 20],'string','0.5','Tag',[typeName 'ContrastEdit']);
hlist = [hlist h];

h = uicontrol(txt,'position',[upperLeft(1)+110+80+50+30+70+200 upperLeft(2)-30 80 20],'string','Flank Dist:','Tag',[typeName 'FlankDistTxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+110+80+50+30+70+80+200 upperLeft(2)-30 50 20],'string','225','Tag',[typeName 'FlankDistEdit']);
hlist = [hlist h];
