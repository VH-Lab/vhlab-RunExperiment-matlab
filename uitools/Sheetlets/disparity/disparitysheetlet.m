function [hlist,lengthwidth] = disparitysheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''disparitysheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''disparitysheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;

hlist = [];

h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 100 20],'string',title,'Tag',[typeName 'disparitytxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];

% h = uicontrol(txt,'position',[upperLeft(1)+100 upperLeft(2)-7 60 20],'string','','Tag',[typeName 'anglestxt'], 'horizontalalignment', 'left');
% hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+170 upperLeft(2)-7 250 20],'string','','Tag',[typeName 'disparityEdit']);
hlist = [hlist h];
