function [hlist,lengthwidth] = binoclineweightsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''binoclineweightsheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''binoclineweightsheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;
...
h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 130 20],'string','BinocLineWeight','Tag',[typeName 'binoclineweighttxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];
...
h = uicontrol(txt,'position',[upperLeft(1)+280 upperLeft(2)-7 60 20],'string','[L W Dur]:','Tag',[typeName 'LWDurtxt'], 'horizontalalignment', 'left');
hlist = [hlist h];
