function [hlist,lengthwidth] = daceyrevcorrsheetlet(typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''daceyrevcorrsheetlet_process'',''' typeName ''');'];
...
cb = txt; cb.Style = 'Checkbox'; cb.Callback = ['sheetlet_callback(''daceyrevcorrsheetlet_process'',''' typeName ''');'];
cb.fontsize = 12;

hlist = [];

h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-10 100 20],'string','Dacey Rev Corr','Tag',[typeName 'Daceytxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];
