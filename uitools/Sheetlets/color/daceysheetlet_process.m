function [varargout]=daceysheetlet_process(fig, typeName, ds, command, varargin)
...
if ~strcmp(command, 'GetVars')&~strcmp(command,'SetVars'),
    [testName,gratCtrl,GoodCB,refCtrl]=daceysheetlet_process(fig, typeName, ds, [typeName 'GetVars']);
end;
...
switch command,
	...
	p.dispprefs = {'BGposttime', isi};
        ...
        dscript = daceyexpandedrevcor(mystim, [p.angle], [p.sFrequency], 0, reps);
    ...
    case 'RestoreVarsBt',
        ...
        g = load(fname,['daceysheetlet' typeName],'-mat');
        if isfield(g,['daceysheetlet' typeName]),
            g=getfield(g,['daceysheetlet' typeName]);
            daceysheetlet_process(fig,typeName,ds,[typeName 'SetVars'],g{:});
        end;
    case 'SaveVarsBt',
        fname = get(findobj(fig,'tag',[typeName 'SaveVarsBt']),'userdata');
        if isempty(fname), error(['Empty filename for ' typeName 'SaveVarsBt']); end;
        eval(['daceysheetlet' typeName '={testName,gratCtrl,GoodCB,refCtrl};']);
        eval(['save ' fname ' daceysheetlet' typeName ' -append -mat']);
    case 'AnalyzeBt',
        ...
        if ~isempty(mycell),
            for j=1:length(mycell),
                ...
                assoc = struct('type',[typeName ' test'],'owner','daceysheetlet','data',testName,'desc',[typeName ' test']);
                assoc(end+1) = struct('type',[typeName ' resp'],'owner','daceysheetlet','data',co,'desc',[typeName ' resp']);
                db.assoc{j} = assoc;
            end;
        end;
