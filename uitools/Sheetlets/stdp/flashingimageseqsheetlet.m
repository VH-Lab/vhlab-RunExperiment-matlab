function [hlist,lengthwidth] = flashingimageseqsheetlet(title,typeName,upperLeft)

button.Units = 'pixels';
button.BackgroundColor = [0.8 0.8 0.8];
button.HorizontalAlignment = 'center';
button.Callback = ['sheetlet_callback(''flashingimageseqsheetlet_process'',''' typeName ''');'];
...
h = uicontrol(txt,'position',[upperLeft(1)+10 upperLeft(2)-7 200 20],'string','Flash Image Sequence','Tag',[typeName 'RCtxt'], 'fontweight', 'bold', 'horizontalalignment', 'left');
hlist = [hlist h];
...
h = uicontrol(txt,'position',[upperLeft(1)+50 upperLeft(2)-30 100 20],'string',['reps isi'],'Tag',[typeName 'Centertxt']);
hlist = [hlist h];

h = uicontrol(edit,'position',[upperLeft(1)+170 upperLeft(2)-30 100 20],'string','','Tag',[typeName 'CenterEdit']);
hlist = [hlist h];

h = uicontrol(txt,'position',[upperLeft(1)+280 upperLeft(2)-30-2 140 20],'string',['Enable reps/isi:'],'Tag',[typeName 'Signtxt'],'visible','on');
hlist = [hlist h];

h = uicontrol(popup,'position',[upperLeft(1)+330 upperLeft(2)-30 70 20],'string',{'---', 'On', 'Off'},'Tag',[typeName 'OnOffPopup'],'visible','off');
hlist = [hlist h];

h = uicontrol(cb,'position',[upperLeft(1)+410+25 upperLeft(2)-28 10 10],'string','','Tag',[typeName 'CenterLocCB'],'visible','on');
hlist = [hlist h];
